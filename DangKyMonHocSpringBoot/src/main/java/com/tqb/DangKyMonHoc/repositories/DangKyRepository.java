...
public interface DangKyRepository extends JpaRepository<DangKy, Integer> {
    ...
    @Query("""
            SELECT new com.tqb.DangKyMonHoc.dto.ThongKeTheoLopDTO(
                bh.id,
                bh.lopId.maLop,
                COUNT(dk.id),
                SUM(CASE WHEN dk.trangThai = 'HOAN_THANH' THEN 1 ELSE 0 END),
                SUM(CASE WHEN dk.trangThai = 'TRUOT' THEN 1 ELSE 0 END)
            )
            FROM DangKy dk
            JOIN dk.buoiHocId bh
            WHERE bh.hocKyId.id = :hocKyId
                AND bh.monHocId.id = :monHocId
            GROUP BY bh.id, bh.lopId
           """)
    List<ThongKeTheoLopDTO> thongKeTheoLop(@Param("hocKyId") int hocKyId, @Param("monHocId") int monHocId);
